...
export default {
    ...
    return new Promise((resolve, reject) => {
      request(URL.GET.replace(':notebookId', notebookId))
        .then(res => {
          res.data = res.data.map(note => {
            note.createdAtFriendly = friendlyDate(note.createdAt)
            note.updateAtFriendly = friendlyDate(note.updateAt)
            return note
          }).sort((note1, note2) => {
            return note1.updateAt < note2.updatedAt
          })
          resolve(res)
        }).catch(err => {
        reject(err)
  ...
  updateNote({noteId}, {title, content}) {
    return request(URL.UPDATE.replace(':notesId', noteId), 'PATCH', {title, content})
  },
  deleteNote({noteId}) {
    return request(URL.DELETE.replace(':notesId', noteId), 'DELETE')
  },
  addNote({notebookId}, {title = '', content = ''} = {title: '', content: ''}) {
    return new Promise((resolve,reject)=>{
     request(URL.ADD.replace(':notebookId', notebookId), 'POST', {title, content}).then(res=>{
       res.data.createdAtFriendly = friendlyDate(res.data.createdAt)
       res.data.updateAtFriendly = friendlyDate(res.data.updateAt)
       resolve(res)
     }).catch(err=>{
       reject(err)
     })
